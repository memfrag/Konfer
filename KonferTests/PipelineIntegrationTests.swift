...
/// setting `KONFER_AUDIO` to a recording:
///
///     KONFER_AUDIO=/path/to/meeting.m4a \
...
struct PipelineIntegrationTests {
    ...
    /// Set `KONFER_LIBRARY=real` to write the result into the app's actual
    ...
    /// `KONFER_LANGUAGE=swedish|english` tags the meeting. The model no longer
    /// follows from it — a mismatch fails the run rather than being rerouted.
    nonisolated static var language: MeetingLanguage {
        ProcessInfo.processInfo.environment["KONFER_LANGUAGE"]
    ...
    func transcribesRealAudio() async throws {
        ...
        let scratch = FileManager.default.temporaryDirectory
            .appendingPathComponent("KonferIntegration-\(UUID().uuidString)")
        ...
        pipeline.fastTranscription =
            ProcessInfo.processInfo.environment["KONFER_FAST"] == "1"
