...
actor WhisperKitBackend: TranscriptionBackend {
    ...
    /// for every language Konfer offers, so there is no second table to keep in
    /// step.
    ///
    ...
    /// `KONFER_CHUNKING` overrides both, for comparison runs.
    nonisolated static func chunkingStrategy(allowed: Bool) -> ChunkingStrategy {
        if let override = ProcessInfo.processInfo.environment["KONFER_CHUNKING"]
    ...
    nonisolated static var voiceActivityDetector: VoiceActivityDetector? {
        ...
        guard let threshold = environment["KONFER_VAD_THRESHOLD"].flatMap(Float.init)
        else { return nil }
        let overlap = environment["KONFER_VAD_OVERLAP"].flatMap(Float.init) ?? 0
    ...
    /// `KONFER_SLICES` overrides it; 1 restores a single pass.
    ...
    nonisolated static func sliceCount(for duration: TimeInterval) -> Int {
        if let override = ProcessInfo.processInfo.environment["KONFER_SLICES"]
            .flatMap(Int.init) {
            return override
        }
    ...
    /// `KONFER_WHISPER_VERBOSE=1` turns on WhisperKit's own logging, which is
    /// the only way to see where a slow first load is actually spending time.
    nonisolated static var verboseLogging: Bool {
        ProcessInfo.processInfo.environment["KONFER_WHISPER_VERBOSE"] == "1"
    }
