...
nonisolated final class AggregateDeviceRecorder: RecordingSource, @unchecked Sendable {
    ...
    func prepare(_ configuration: RecordingConfiguration) async throws {
        ...
        tapDescription.name = "Snoopy \(application.name)"
        ...
        let description: [String: Any] = [
            kAudioAggregateDeviceNameKey: "Snoopy Recording",
            kAudioAggregateDeviceUIDKey: "pizza.martin.Snoopy.aggregate.\(UUID().uuidString)",
    ...
    /// `SNOOPY_RECORD_DIAGNOSTICS=1` reports what the aggregate device actually
    /// produced, which is the only way to tell "captured silence" apart from
    /// "captured nothing".
    nonisolated static var isDiagnostic: Bool {
        ProcessInfo.processInfo.environment["SNOOPY_RECORD_DIAGNOSTICS"] == "1"
    }
